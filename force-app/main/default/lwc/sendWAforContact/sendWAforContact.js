...
import get_details from '@salesforce/apex/SendWAforContacts.tempwap';
import on_save from '@salesforce/apex/SendWAforContacts.onsave';
import deletedoc from '@salesforce/apex/SendWAforContacts.deletdoc';
import getDocumentSize from '@salesforce/apex/SendWAforContacts.getDocumentSize1';
import { getRecord, getFieldValue } from "lightning/uiRecordApi"; 
import Name from "@salesforce/schema/Contact.Name";  
import WHATSUP_NUMBER from "@salesforce/schema/Contact.WhatsApp_No__c";  
const fields = [Name, WHATSUP_NUMBER];
export default class SendWAforContact extends NavigationMixin(LightningElement) {
    @track attachments = [];
    @api recordId;
    @track emailbody = '';
    ...
    onCallback_GetContactDetails(response){
        ...
        if (data) {
            ...
            console.log('@@@@@@@@@@@vishla whatsupnumber@@@@@@');
            this.whatsUpNumber  = data.fields.WhatsApp_No__c.value;
            console.log(JSON.stringify(this.whatsUpNumber));
            console.log(!this.whatsUpNumber);
            this.blocksave = !this.whatsUpNumber; // Disable if null
        } else if (error) {
...
async deletedoc123(docid){
    deletedoc({Docid:docid}).then(result=>{
    ...
    handleRemoveAttachment(event) {
        const attachmentIndex = event.target.dataset.attachmentIndex;
          this.deletedoc123(this._lstAttachments_master[attachmentIndex].documentId);
