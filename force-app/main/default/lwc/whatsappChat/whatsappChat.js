...
import getChats from '@salesforce/apex/WhatsAppChatController.getChats';
import getTemplates from '@salesforce/apex/WhatsAppChatController.getTemplates';
import sendTemplate from '@salesforce/apex/WhatsAppChatController.SendTemplate';
import sendMessage from '@salesforce/apex/WhatsAppChatController.sendMessage';
import sendMessageWithAttchments from '@salesforce/apex/WhatsAppChatController.sendMessageWithAttchments';
import UploadFiles from '@salesforce/apex/WhatsAppChatController.UploadFiles';
import getImageUrl from '@salesforce/apex/WhatsAppChatController.getImageUrl';
import get_details from '@salesforce/apex/WhatsAppChatController.tempwap';
import getDate from  '@salesforce/apex/WhatsAppChatController.dateSend';
...
export default class WhatsappChat extends NavigationMixin(LightningElement) {
    ...
    handleButtonClick() {
        // Reset the select field value by directly manipulating the DOM
    ...
    /*@track imageUrl = ''
    @track cvId = '';
    @track showTitleMessage = '';
    @track iffileReplyBack = false;
    handlePreviewFile(event){
        this.iffileReplyBack = true;
        this.cssStyle = "width:90%;border-left: 5px solid #25D366;"
        this.headerColor = "color:#25D366";
        const sename = event.target.dataset.name;
        this.sname = sename;

        const documetId = event.target.dataset.fileid;
        const titleFile = event.target.dataset.title;
        console.log('Title Name >> ' + titleFile);
        this.showTitleMessage = titleFile;
         getImageUrl({ contentVersionId: documetId })
            .then(result => {
